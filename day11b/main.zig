...
const Monkey = struct {
    items: ArrayList(u64) = ArrayList(u64).init(gpa.allocator()),
    inspections: u32 = 0,
    operation: ?u64 = null,
...
pub fn main() !void {
    ...
    while (lines.next()) |line| {
        ...
        if (eql(u8, line[2..16], "Starting items")) {
            ...
            while (items.next()) |item| try cur_monkey.items.append(try std.fmt.parseUnsigned(u64, item, 10));
            continue;
        }
        if (eql(u8, line[2..11], "Operation")) {
            var op = std.mem.splitBackwards(u8, line, " ");
            var operation = op.next().?;
            cur_monkey.operation = if (!eql(u8, operation, "old")) try std.fmt.parseUnsigned(u64, operation, 10) else null;
        ...
        if (eql(u8, line[2..6], "Test")) {
            ...
            common_divider *= cur_monkey.op_test;
            continue;
        }
    ...
    var inspections = ArrayList(u64).init(gpa.allocator());
    defer inspections.deinit();
    for (monkeys.items) |monkey| {
        try inspections.append(monkey.inspections);
    }
    std.sort.sort(u64, inspections.items, {}, std.sort.desc(u64));
